...
# ---- Create config early (so we can use APN for ppp-creator) ----
mkdir -p /etc/rak2013
...
. /etc/rak2013/cellular.conf || true
...
# ---- FIX: permissions BEFORE checking/using ppp-creator.sh ----
...
if [ -x "${RAK_PPPD_DST}/ppp-creator.sh" ]; then
  if [ -n "${APN:-}" ]; then
    "${RAK_PPPD_DST}/ppp-creator.sh" "${APN}" "${TTY_DEV}" "${TTY_SPEED}"
    echo "Generated /etc/ppp/peers/${PPP_PEER} and chatscripts."
  else
    echo "INFO: APN is empty in /etc/rak2013/cellular.conf. Skipping ppp-creator.sh."
    echo "      Set APN and restart service: sudo nano /etc/rak2013/cellular.conf && sudo systemctl restart rak2013-ppp"
  fi
else
